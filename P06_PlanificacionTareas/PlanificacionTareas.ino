...
float angGrados = 0; // Ángulo en grados
float angRadianes = 0; // Ángulo en radianes
                  ...
                  while (Serial.available()) {
                    ...
                    if (v == '\n') { // Fin del comando
                      ...
                      if (ntaylor > 0) {
                        ...
                        angGrados = ntaylor; // Asigna el valor del ángulo introducido a la variable
...
void TaskSerieTaylor(void *parameter) {
  ...
  while (1) {
      angRadianes = angGrados * (3.14159 / 180.0); // Conversión de ángulo de grados a radianes
      seno = 0; // Variable para resultado
      for (int i = 0; i <= limite; i++) {  // Ciclo para el cálculo del polinomio de Taylor de grado 20
        float valor = pow(-1,i)*pow(angRadianes, 2*(i+1)) / tgamma(2 *(i+2)); // Valor del término de Taylor
        seno = seno+valor;
